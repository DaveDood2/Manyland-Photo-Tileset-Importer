...
def doExtractPixels(INPUT_FILE, roomName = ""):
    ...
    cellWidth = 38 # Assuming cell width is same as cell height
    ...
    if (roomName == ""):
        ...
        f = open("ROOMOutput\\FAR" + OUTPUT_FILE + ".js", "w")
    else:
        changerCode = toChanger(toOutput, roomName)
        f = open("ROOMOutput\\FAR" + roomName + ".js", "w")
    f.write(changerCode)
    f.close()
...
function draw(creation){
    ...
    if (name.includes("[") && name.includes("]")){
        ...
        offsetX = parseInt(offsetX) * 114 //Offset multiplied by 114 to account for the width/height of a 3x3 far back!
        offsetY = parseInt(offsetY) * 114
    }
    ...
    return creation;
        ...
        function colour(x, y, cellx, celly){
            var colourtext = getdata(x + cellx * 38, y + celly * 38);
...
if __name__ == "__main__":
>>>>>>> ce5e6e2c842687a60e66b9dc1ac874edc696fe50
  main(sys.argv[1:])